  ...
  public DistortionCorrectionMethod DistortionCorrection {
    ...
    set {
      if (device != null && device.RequiresNativeDistortionCorrection()) {
				value = DistortionCorrectionMethod.None;                     //Fix
      }
      if (value != distortionCorrection && device != null) {
				device.SetDistortionCorrectionEnabled(value == DistortionCorrectionMethod.None   //Fix
            && NativeDistortionCorrectionSupported);
//				device.SetDistortionCorrectionEnabled (false);         //add
  ...
  [SerializeField]
	...
	private DistortionCorrectionMethod distortionCorrection = DistortionCorrectionMethod.None;     //Fix
#else
	private DistortionCorrectionMethod distortionCorrection = DistortionCorrectionMethod.None;    //Fix
  ...
  public RenderTexture StereoScreen {
    get {
      // Don't need it except for distortion correction.
			if (distortionCorrection == DistortionCorrectionMethod.None || !VRModeEnabled) {     //Fix
        return null;
      }
  ...
  /// takes care of adjusting the matrix for its particular camera.
	public Matrix4x4 Projection(Eye eye, Distortion distortion = Distortion.Undistorted) {               //Fix
  ...
  /// screen that the eye can see.
	public Rect Viewport(Eye eye, Distortion distortion = Distortion.Undistorted) {            //Fix
